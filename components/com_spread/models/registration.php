 ...
 * @since		1.6
 */
class SpreadModelRegistration extends JModelForm
{
	/**
    ...
    public function &getBkImg()
    {
    	
        if (!empty($this->_bk_img))
        {
            return $this->_bk_img;
        }

        $db = JFactory::getDBO();
        $query = "SELECT spread_bk FROM #__games WHERE guid=".JRequest::getVar('g');
        $db->setQuery($query);
        $res = $db->loadObjectList();
        $this->_bk_img = $res[0]->spread_bk;
