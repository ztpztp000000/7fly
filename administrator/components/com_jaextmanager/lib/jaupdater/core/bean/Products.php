	...
	function getInfoFile()
	{
		...
		switch ($this->type) {
			...
			case 'module':
				...
				break;
			case 'plugin':
				$path = dirname($location[$this->extKey]) . DS;
				break;
			case 'template':
				if (isset($location['location']))
					$path = $location['location'];
	...
	function _applyPackage($src, $objectFilter, $context = 'upgrade')
	{
		...
		switch ($this->type) {
			...
			case 'component':
				...
				break;
			case 'plugin':
				//echo $src . $this->extKey;exit();
				if (isset($locations[$this->extKey]) && JFolder::exists($src)) {
					$FileSystemHelper->cp($src, $locations[$this->extKey], true);
					if (isset($objectFilter)) {
						$this->_applyFileRemoved($objectFilter, $locations[$this->extKey]);
					}
				}
