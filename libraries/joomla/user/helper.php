...
abstract class JUserHelper
{
	 ...
	 * @since   11.1
	 */
	public static function getCryptedPassword($plaintext, $salt = '', $encryption = 'sha1', $show_encrypt = false)
	{
		// Get the salt to use.
		...
		{
			case 'sha1':
				$encrypted = ($salt) ? sha1($plaintext . $salt) : sha1($plaintext);
                return ($show_encrypt) ? '{SHA1}' . $encrypted : $encrypted;
			case 'plain':
				return $plaintext;
