...
class JUser extends JObject
{
	...
	public function save($updateOnly = false)
	{
		...
		try
		{
			...
			{
			    /// 不能修改用户名
	            if ($oldUser->username != $table->get('username'))
	            {
	                $this->setError('不能修改用户名');
	                return false;
	            }
	            
	            /// 邮箱没有修改时不检察
                if ($oldUser->email == $table->get('email'))
                {
                    $table->set('email', '');
                }
			}
			...
			/*
			if (empty($table->get('email')))
			{
				$table->set('email', $oldUser->email);
			}*/
			...
			{
				/// 注册到总站成功才保存到本地
				$regUser = $ms_api->register($table->get('username'), $table->get('password'), $table->get('email'), 0);
				if (1 != $regUser['ret'])
				{
					$this->setError($regUser['msg']);
					return false;
				}
			}
			else
			{
				/// 更新密码
				$ret = $ms_api->update_pwd($table->get('username'), $table->get('password'));
		        if (1 != $ret['ret'])
		        {
