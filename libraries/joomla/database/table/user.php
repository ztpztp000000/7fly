...
class JTableUser extends JTable
{
	...
	public function check()
	{
		...
		$ms_api = new MainSiteApi($config->get('api_url'), $config->get('site_id'), $config->get('site_key'));
		if (empty($this->id))
		{
			// 新用户检查用户名和邮箱
			$reg_s = $ms_api->check_user('username', $this->username);
			if (1 != $reg_s)
			{
				$this->setError(JText::_('JLIB_DATABASE_ERROR_USERNAME_INUSE'));
				return false;
		    ...
		    $reg_s = $ms_api->check_user('email', $this->email);
            if (1 != $reg_s)
            {
